...
public class GameMgr : BaseManager<GameMgr>
{
    ...
    private Vector3 locationItem(int i, GameObject parentObj, GameObject _obj)
    {
        ...
        float x2 = x1 + (_objRect.rect.width+5) * 2;

        float y1 = parentObj.transform.localPosition.y + parentRect.rect.height / 2 - _objRect.rect.height * 3 / 4 - 5;
        float y2 = y1 - (_objRect.rect.height+5) * 2;
  
        _vv3.x = i < 3 ? (x1 + (_objRect.rect.width+5) * i) : i < 5 ? x2 : i < 7 ? (x2 - (_objRect.rect.width + 5) * (i - 4)) : x1;
        _vv3.y = i < 3 ? y1 : i < 5 ? y1 - (_objRect.rect.height + 5) * (i - 2) : i < 7 ? y2 : (y2 + (_objRect.rect.height + 5) * (i - 6));
        return _vv3;
    }
