    ...
    public void ApplyItemInObj(GameObject parentObj, SItemData itemData)
    {
        ...
        for (int i = 0; i < gameData.proCount; i++)
        {
            ...
            poolMgr.GetObj(gameData.preName, gameData.preUrl, (obj) =>
            {
                ...
                //(locationItem(gameData, indexI - 1, parentObj, _obj), new Quaternion(0, 0, 0, 0));
                _obj.transform.localPosition = locationItem(gameData, indexI - 1, parentObj, _obj);
            });
        }
    }
    private Vector3 locationItem(GameDate gameData,int i, GameObject parentObj, GameObject _obj)
    {
        Vector3 _vv3 = new Vector3();
        _vv3.x = gameData.PosArr[i,0];
            _vv3.y= gameData.PosArr[i, 1];
        /* RectTransform parentRect = parentObj.GetComponent<RectTransform>();
         RectTransform _objRect = _obj.GetComponent<RectTransform>();

         float x1 = parentObj.transform.localPosition.x - parentRect.rect.width / 2 + _objRect.rect.width * 2 / 3 + 10;
         float x2 = x1 + _objRect.rect.width * 3+10;

         float y1 = parentObj.transform.localPosition.y + parentRect.rect.height / 2 - _objRect.rect.height ;
         float y2 = y1 - (parentRect.rect.height / 4 + 19) * 2;

         _vv3.x = i < 4 ? (x1 + (_objRect.rect.width + 10) * i) : i < 6 ? x2 : i < 10 ? (x2 - (_objRect.rect.width + 10) * (i - 5)) : x1;
         _vv3.y = i < 4 ? y1 : i < 6 ? y1 - (_objRect.rect.height + 10) * (i - 3) : i < 10 ? y2 : (y2 + (_objRect.rect.height + 10) * (i - 9));*/
        return _vv3;
    }
