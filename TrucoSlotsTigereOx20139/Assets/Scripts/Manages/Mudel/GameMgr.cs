...
public class GameMgr : BaseManager<GameMgr>
{
    ...
    private Vector3 locationItem(int i, GameObject parentObj, GameObject _obj)
    {
        ...
        float x1 = parentObj.transform.localPosition.x - (parentRect.rect.width * 3 - parentRect.rect.width) / 6 ;
        float x2 = x1 + (parentRect.rect.width / 3 + 2) * 2;

        float y1 = parentObj.transform.localPosition.y + parentRect.rect.height / 4 + 16;
        float y2 = y1 - (parentRect.rect.height / 3) * 2;

        _vv3.x = i < 3 ? (x1 + (parentRect.rect.width / 3 + 2) * i) : i < 5 ? x2 : i < 7 ? (x2 - (parentRect.rect.width / 3 + 2) * (i - 4)) : x1;
        _vv3.y = i < 3 ? y1 : i < 5 ? y1 - (parentRect.rect.height / 3) * (i - 2) : i < 7 ? y2 : (y2 + (parentRect.rect.height / 3) * (i - 6));
        return _vv3;
    }
