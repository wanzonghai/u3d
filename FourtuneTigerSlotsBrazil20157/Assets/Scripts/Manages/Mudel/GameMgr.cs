...
public class GameMgr : BaseManager<GameMgr>
{
    ...
    private Vector3 locationItem(int i, GameObject parentObj, GameObject _obj)
    {
        ...
        float x1 = parentObj.transform.localPosition.x - parentRect.rect.width / 2 + _objRect.rect.width*3/4-10;
        float x2 = x1 + (_objRect.rect.width + 5) * 4;

        float y1 = parentObj.transform.localPosition.y + parentRect.rect.height / 4 - 16;
        float y2 = y1 - (_objRect.rect.height) * 2;
  
        _vv3.x = i < 5 ? (x1 + (_objRect.rect.width + 5) * i) : i < 7 ? x2 : i < 11 ? (x2 - (_objRect.rect.width + 5) * (i - 6)) : x1;
        _vv3.y = i < 5 ? y1 : i < 7 ? y1 - (_objRect.rect.height) * (i - 4) : i < 11 ? y2 : (y2 + (_objRect.rect.height) * (i - 10));
        return _vv3;
    }
