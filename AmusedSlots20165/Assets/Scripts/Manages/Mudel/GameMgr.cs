...
public class GameMgr : BaseManager<GameMgr>
{
    ...
    private Vector3 locationItem(int i, GameObject parentObj, GameObject _obj)
    {
        ...
        float x1 = parentObj.transform.localPosition.x - parentRect.rect.width / 2 + _objRect.rect.width/2+32;
        float x2 = x1 + (_objRect.rect.width - 13)  * 7;
        
        float y1 = parentObj.transform.localPosition.y + parentRect.rect.height / 2 - _objRect.rect.height / 2 - 73;
        float y2 = y1 - (_objRect.rect.height - 7) * 3;

        _vv3.x = i < 8 ? (x1 + (_objRect.rect.width - 13) * i) : i < 11 ? x2 : i < 18 ? (x2 - (_objRect.rect.width - 13)  * (i - 10)) : x1;
        _vv3.y = i < 8 ? y1 : i < 11 ? y1 - (_objRect.rect.height-7 ) * (i - 7) : i < 18 ? y2 : (y2 + (_objRect.rect.height -7) * (i - 17));
        return _vv3;
    }
