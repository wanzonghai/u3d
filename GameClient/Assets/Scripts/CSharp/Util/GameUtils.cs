...
public class GameUtils
{
    ...
    public static IEnumerator LoadABBytes()
    {
        ...
        {
            UnityWebRequest webRequest = UnityWebRequestAssetBundle.GetAssetBundle(GetWWWFullPathByMd5("scripts/config.bytes"));
            ...
            webRequest = UnityWebRequestAssetBundle.GetAssetBundle(GetWWWFullPathByMd5("scripts/lua.bytes"));
            yield return webRequest.SendWebRequest();
        ...
        else
        {
            ...
            ConfigFileName.AddRange(Directory.GetFiles("Assets/Scripts/Config/", "*.bytes", SearchOption.AllDirectories));
            for (int i = 0; i < ConfigFileName.Count; i++)
            {
                ConfigFileName[i] = ConfigFileName[i].Replace("\\", "/");
            }
            CodeFileName = new List<string>();
            CodeFileName.AddRange(Directory.GetFiles("Assets/Scripts/Lua/", "*.bytes", SearchOption.AllDirectories));
    ...
    public static byte[] ReadAllBytesByBundle(ref string path)
    {
        ...
        if (path.StartsWith("Config/"))
        {
            ...
            {
                path = "Assets/Scripts/" + path + ".bytes";
        ...
        else
        {
            if (CodeBundle == null)
            {
                path = "Assets/Scripts/Lua/" + path + ".bytes";
